...
class SettingsService extends BoxServiceBase<SettingsModel>
  ...
  Future<void> loadSettings(ThemeChanger themeChanger) async {
    ...
    if (list.isNotEmpty) {
      ...
      // primary color
      if(_settings.primaryColor != 0) themeChanger.setPrimaryColor(Color(_settings.primaryColor));
      // second color
      if(_settings.secondaryColor != 0) themeChanger.setSecondColor(Color(_settings.secondaryColor));
    } else {
      _settings = SettingsModel(false, true, false, false, 0, 0);
